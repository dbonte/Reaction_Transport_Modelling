...
vignette: >
  %\VignetteIndexEntry{Equilibrium chemistry I, one equilibrium reaction: modeling exercise}
...
Suppose we have a system with chemical species A, B and AB (such as dissolved NH$_3$, H$^+$ and NH$_4^+$, respectively) that undergo the following processes: 
   ...
   - (i) species AB can dissociate into A and B at a rate controlled by the rate constant $k_f$ ("forward reaction"), 
   - (ii) species A and B can react to form species AB at a rate controlled by the rate constant $k_b$ ("backward reaction"), and 
   - (iii) species A is removed from the system at a rate controlled by the rate constant $\lambda$ ("net removal process"). 
...
These parameters imply an equilibrium constant of $K_{eq}=0.5e^{-3}$  $mol~m^{-3}$.
...
plot(out.full, mfrow=c(2,3), ylab="mol/m3")
```
...
You can start with the R-markdown template model file *RTM_equilibrium.Rmd* \footnote{you can download this file from Rstudio: "File->new File->Rmarkdown..->from template->RTM_equilibrium". Save this file under a different name, do not forget to change the heading of this file.}
