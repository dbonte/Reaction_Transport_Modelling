...
* The oxygen exchange with the atmosphere (aeration) is driven by the "distance" from the equilibrium. Thus, the aeration rate (mol $m^{-3}$ $d^{-1}$) is expressed as 
$$Aeration = - \frac{v_d}{d} \cdot ([O_2] - O_{2,sat}).$$
This expression assumes that the aeration flux is *instantaneously* homogenized over the entire water depth ($d$). Assume a value of $v_d = 1~m~d^{-1}$ for the rate constant (called 'piston-velocity'), and $O_{2,sat} = 300~\mu mol~O_2~L^{-1}$ for the equilibrium concentration of dissolved oxygen in water (called 'solubility'). 

* The model is completely specified only if we prescribe what happens at the boundaries with the outside world (i.e., the boundary conditions). For the estuary, the outside world is the river (distance $x = 0$) and the sea (distance $x = 100~km$).  In the current situation, the concentrations of oxygen, nitrate and ammonia in the river are 0.1, 0.3 and $0.1~mol~m^{-3}$, respectively. At the sea boundary, they are 0.3, 0.05 and $0.010~mol~m^{-3}$, respectively.
...
* Implement the model, using the R-markdown file RTM_1D.Rmd as a template\footnote{you can download this file from Rstudio: "File->new File->Rmarkdown..->from template->RTM_1D.  Save this file under a different name, do not forget to change the heading of this file.}.
...
* Run the model again using these different boundary conditions:
    - For nitrate, use 0.050 and $0.020~mol~m^{-3}$ as concentrations in the river and sea, respectively.
    - For ammonia, use 0.600 and $0.020~mol~m^{-3}$ as a concentrations in the river and sea, respectively.
