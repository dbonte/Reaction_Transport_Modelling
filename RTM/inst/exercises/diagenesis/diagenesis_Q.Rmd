...
* Add ammonia, O2 and Nitrate as state variables - you can use the dynamics of DIC as a template as this is also a dissolved substance.
     ...
     - you will now model 4 species (rather than 2) => *nspec = 4*
...
* run the model for three cases, C deposition (depoPOC) = 1$e^{-3}$ $m^{-2}d^{-1}$, the default, and depoPOC = 1$e^{-2}$ mol $m^{-2}d^{-1}$ and depoPOC = 2$e^{-2}$ mol $m^{-2}d^{-1}$.
...
For each of the three runs : what is the relative contribution of the oxic mineralisation, denitrification and sulphate reduction to the total mineralisation.

## Task 5: O2 budget

Create a budget for $O_2$ for the three runs.

Which processes consume most oxygen in the three runs?
