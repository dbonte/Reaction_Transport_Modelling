...
*	All state variables are expressed in $molN~m^{-3}$.
...
| ksDIN	         | 1$e^{-3}$ | Monod ct for nutrient limitation | $mol~N~m^{-3}$              |
| rGrazing	     | 1.0	     | Grazing rate constant            | $d^{-1}$                    |
| ksGrazing	     | 1$e^{-3}$ | Monod ct for grazing             | $mol~N~m^{-3}$              |
...
* Implement the model in R, using the R+markdown file *RTM_npzd.Rmd* to start with (this file already contains the statement that estimates the light intensity as a function of the time of the year and the depth of the water column)\footnote{you can download this file from Rstudio: "File->new File->Rmarkdown..->from template->NAME  Save this file under a different name, do not forget to change the heading of this file.}. First rename this file to *NPZD.Rmd*.
