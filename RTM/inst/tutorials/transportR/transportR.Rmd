...
$$
\frac{\partial C}{\partial t} = \frac{1}{1-\phi_x} \frac{\partial}{\partial x}\left[(1-\phi_x) D_b \frac{ \partial C}{\partial x}\right] - r \cdot C
$$
where $C$ is in units of $mol~C~m^{-3}_S$, $x$ is sediment depth in $m$, $t$ is time in $days$, $\phi_x$ is the sediment porosity, $D_b$ is the bioturbation mixing coefficient, and $r$ is the rate constant describing POC mineralization (mineralization is assumed to be described by first-order kinetics).
...
Cmodel <- function (t, C, pars)   # C is a LONG vector
{
  with (as.list(pars),{ 
    ...
    Tran.C <- tran.1D(C = C, flux.up = depoPOC,  # upper boundary: flux
                      dx=Grid, VF=1-por, D=Db, v=v)     # grid, volume fraction, mixing 
...
## NOTE: FOR SOME REASON THE ABOVE CODE NEEDS TO BE COPIED AGAIN ...
require(ReacTran)
Length    <- 0.20   # [m]
N         <- 200
Grid      <- setup.grid.1D(L = Length, N = N, dx.1 = 0.05/100)
...
DICmodel <- function (t, y, pars)   # y is a LONG vector
{
  with (as.list(pars),{
    ...
    Tran.POC   <- tran.1D(C = POC, flux.up = depoPOC, # upper boundary: flux
                      dx=Grid, VF=1-por, D=Db, v=v)  # grid, volume fraction, bioturbation 
