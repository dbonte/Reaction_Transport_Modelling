---
title: "RTM course Course - Conceptual Diagrams"
...
question("When creating a conceptual model, we additionally need to specify", type = "learnr_checkbox",
 ...
 incorrect = "For the conceptual model, we only need to specify the state variables and the spatial and temporal domain, including the domain boundaries. The values of the model parameters, initial conditions and boundary conditions are specified when we *apply* the model.",
 correct = "For the conceptual model, we only need to specify the state variables and the spatial and temporal domain, including the domain boundaries. The values of the model parameters, initial conditions and boundary conditions are specified when we *apply* the model.")
```
...
question("What would be a good model currency for such a model?", type = "learnr_radio",
 ...
 incorrect = "Carbon is used both for biomass and energy production, so this is the best model currency.",
 correct = "Carbon is used both for biomass and energy production, so this is the best model currency."
 )
```
