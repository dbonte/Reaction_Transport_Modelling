...
The table above shows that the P fluxes (in $\mu mol~P~m^{-2}~d^{-1}$) are in a much narrower interval if predicted by the model constrained by the new data (Sd is much smaller). For example, out of the P flux that enters the sediment due to the sedimentation of organic matter (`r round(out.new["POP.SWI.Flux","Mean"]*1e6,3)` $\mu mol~P~m^{-2}~d^{-1}$), about `r round(-out.new["PO4.SWI.Flux","Mean"]*1e6,4)` $\pm$ `r round(out.new["PO4.SWI.Flux","Sd"]*1e6,4)` $\mu mol~P~m^{-2}~d^{-1}$ leaves the sediment at the SWI in the form of $PO_4$, while about `r round(out.new["Pads.Deep.Flux","Mean"]*1e6,4)` $\pm$ `r round(out.new["Pads.Deep.Flux","Sd"]*1e6,4)` $\mu mol~P~m^{-2}~d^{-1}$ is buried in the deeper sediment in the form of $P_{ads}$.
